...
def simulate_transmission(env, graph, use_zkp=True):
    # Key Generation
    alice_rsa_key_pair = generate_rsa_key_pair()
    bob_rsa_key_pair = generate_rsa_key_pair()
    symmetric_key = generate_aes_key()

    # Key Exchange: Alice encrypts the symmetric key with Bob's public key
    encrypted_symmetric_key = rsa_encrypt(symmetric_key, bob_rsa_key_pair.publickey())
    ...
    decrypted_symmetric_key = rsa_decrypt(encrypted_symmetric_key, bob_rsa_key_pair)
    ...
    env.process(network_simulation(env, "Alice", receiver_channel, encrypted_symmetric_key, graph))
    env.process(receiver_simulation(env, receiver_channel, bob_rsa_key_pair, graph))
...
# Simulate transmission with ZKP
...
# Simulate transmission without ZKP
